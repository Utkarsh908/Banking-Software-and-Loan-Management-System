...
class LoanManager:
    def __init__(self, root):
        ...
        title = Label(self.root, text="Loan Management System", font=("Comic Sans MS", 20, ), bd=8,  bg='black', fg='white')
        ...
        # Create a frame to hold the loan type radio buttons
        loan_type_frame = Frame(self.root, bd=4, relief=RIDGE, bg='white')
        loan_type_frame.place(x=10, y=40, width=520, height=40)

        # Create radio buttons for different loan types
        car_loan_radio = Radiobutton(loan_type_frame, text="Car Loan", variable=self.loan_type, value="Car Loan")
        car_loan_radio.grid(row=0, column=0, padx=20, pady=5)

        two_wheeler_loan_radio = Radiobutton(loan_type_frame, text="Two Wheeler Loan", variable=self.loan_type, value="Two Wheeler Loan")
        two_wheeler_loan_radio.grid(row=0, column=1, padx=20, pady=5)

        home_loan_radio = Radiobutton(loan_type_frame, text="Home Loan", variable=self.loan_type, value="Home Loan")
        home_loan_radio.grid(row=0, column=2, padx=20, pady=5)

        personal_loan_radio = Radiobutton(loan_type_frame, text="Personal Loan", variable=self.loan_type, value="Personal Loan")
        personal_loan_radio.grid(row=0, column=3, padx=20, pady=5)

        # Set default loan type to Car Loan
        self.loan_type.set("Car Loan")
        self.loan_type = StringVar()
        ...
        # Create radio buttons for Car Loan and Two Wheeler Loan
        car_loan_radio = Radiobutton(loan_type_frame, text="Car Loan", variable=self.loan_type, value="Car Loan")
        car_loan_radio.grid(row=0, column=0, padx=20, pady=5)
    ...
    def total(self):
        ...
        if self.loan_type.get() == "Car Loan":
            # Car Loan eligibility criteria
            min_balance_criteria = 20000
            interest_rate = 0.06
            late_payment_interest_rate = 0.08
            max_loan_duration = 60
        elif self.loan_type.get() == "Two Wheeler Loan":
            # Two Wheeler Loan eligibility criteria
            min_balance_criteria = 5000
            interest_rate = 0.04
            late_payment_interest_rate = 0.06
            max_loan_duration = 60
        else:
            # Default values if no loan type is selected
            min_balance_criteria = 0
            interest_rate = 0
...
class Login():
    ...
    def login(self):
        ...
        if self.username.get() == "utkarsh" and self.password.get() == "utkarsh":
